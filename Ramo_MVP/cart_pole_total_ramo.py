...
for run in range(1):
    ...
    # First conv layer
    first= Dense(8, activation='relu')(input_layer)
    second = Dense(8, activation='relu')(first)
    # Output layers
    output_actor  = Dense(units=NUM_ACTIONS, activation='softmax')(second)
    output_critic = Dense(units=1, activation='linear')(second)
    # The "final" networks
    ...
    opt = tf.train.AdamOptimizer(LR)#, decay=ALPHA, epsilon=EPSILON)
    ...
    a_t = tf.placeholder(tf.float32, shape=(None, NUM_ACTIONS), name="actions")
    r_t = tf.placeholder(tf.float32, shape=(None, 1), name="reward")
    advantage = tf.placeholder(tf.float32, shape=(None, 1), name="advantages")

    p, v = global_model(s_t)
    #self.advantage = (self.r_t - v)
    
    weights = global_model.trainable_weights[::2] # weight tensors
    # Actor weights
    ...
    loss_actor = -tf.reduce_mean(tf.reduce_sum(tf.multiply(log_p, a_t), reduction_indices=1) * advantage)
    loss_critic = 0.5 * tf.reduce_mean(tf.square(r_t - v))
    ...
    with graph.as_default():
        ...
        grads_var = list(zip(grads, weights))
        update = opt.apply_gradients(grads_var)
                ...
                while not done:
                    ...
                    if self.step_counter - t_start == t_max or done:
                        ...
                        for s, a, r, d, v in memory:
                            ...
                            s = s[0]
                            state_list.append(s)
                            advantage_tmp = np.reshape(R - v, (-1, 1))
                            advantages.append(advantage_tmp[0])
                        state_list  = state_list[::-1]
                        action_list = action_list[::-1]
                        return_list = return_list[::-1]
                        advantages  = advantages[::-1]
                        sess.run(update, {s_t : state_list,
                                               a_t : action_list,
                                               r_t : return_list,
