    ...
    def train(self, t, episodes):
        ...
        while i < episodes: 
            ...
            while not done:
                ...
                log_prob = np.array([np.array([(probabilities[0][action])])])
                ...
                while j < len(actor_grads):
                    ...
                    # Actor
                    update_actor = self.local_actor_model.layers[j+1].get_weights()[0] + ALPHA * I * td_error * actor_grads[j] / probabilities[0][action]
